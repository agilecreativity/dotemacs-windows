...
;; TODO: may be required it to 'use-package instead if possible
(require 'imenu)
...
;; TODO: maybe cleaner if we write this as advice instead?
;; See: [[ftp://ftp.gnu.org/old-gnu/Manuals/elisp-manual-20-2.5/html_chapter/elisp_17.html][Advice example]]
(defadvice split-window-right (after move-to-new-window ())
  "Make the new window active after the split."
  (windmove-right))

(ad-activate 'split-window-right)

;; Move to the split windows after split
(defadvice split-window-below (after move-to-new-window ())
  "Make the new window active after the split."
  (windmove-down))
...
(defun indent-buffer ()
  "Indent the current buffer by calling indent-region on the whole buffer."
  (interactive)
